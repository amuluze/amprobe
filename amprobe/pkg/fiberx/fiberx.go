...
func Failure(c *fiber.Ctx, err errors.Error) error {
	...
	if status := err.Status; status >= 400 && status < 500 {
		slog.Warn("return 4xx error", "error", err.Error)
	} else if status >= 500 {
		slog.Error("return 5xx error", "error", err.Error)
	}

	return ReturnJson(c, err.Status, &FailedResponse{Err: err.Error, Msg: err.Message})
}
