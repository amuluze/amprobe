...
type Role struct {
	...
	CreatedAt time.Time `gorm:"comment:创建时间"`
	UpdatedAt time.Time `gorm:"comment:更新时间"`
	Name      string    `gorm:"size:64;uniqueIndex;default:;not null;comment:角色名称"`
	Status    int       `gorm:"index;default:0;comment:状态(1:启用 2:停用)"`
	Remark    string    `gorm:"size:200;default:;comment:备注"`
...
type Resource struct {
	...
	CreatedAt time.Time `gorm:"comment:创建时间"`
	UpdatedAt time.Time `gorm:"comment:更新时间"`
	Name      string    `gorm:"size:64;uniqueIndex;default:;not null;comment:资源名称"`
	Path      string    `gorm:"size:512;comment:资源路径"`
	Method    string    `gorm:"comment:资源请求方法"`
	Status    int       `gorm:"status;default:1;not null;comment:资源状态(1: 启用 2: 禁用)"`
}

func (a Resources) TableName() string {
	return "sys_resource"
}
