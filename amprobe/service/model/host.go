...
func (d *CPU) AfterCreate(tx *gorm.DB) error {
	tx.Unscoped().Where("timestamp < ?", time.Now().Add(-time.Hour*24*7)).Delete(&CPU{})
	return nil
}
...
func (d *Memory) AfterCreate(tx *gorm.DB) error {
	tx.Unscoped().Where("timestamp < ?", time.Now().Add(-time.Hour*24*7)).Delete(&Memory{})
	return nil
}
...
func (d *Disk) AfterCreate(tx *gorm.DB) error {
	tx.Unscoped().Where("timestamp < ?", time.Now().Add(-time.Hour*24*7)).Delete(&Disk{})
	return nil
}
...
func (d *Net) AfterCreate(tx *gorm.DB) error {
	tx.Unscoped().Where("timestamp < ?", time.Now().Add(-time.Hour*24*7)).Delete(&Net{})
	return nil
}
